...
// Global data
sgx_enclave_id_t eidcount[_MAX];
int updated = 0;
int position = 0;
/*
message 构造函数
...
int httpServer::handleMessage(message msg) 
{
	/*为线程单独声明enclave并创建*/
	//创建enclave空间(废弃)
	/*
	...
	if (ret != SGX_SUCCESS)
	{
		...
		eidcount[msg.id] = eid;
		std::cout << eid << token;
		//printf("%d,%d", eid, token);
		hendlemessage_enclave_process(eid,&msg);
	}
	//printf("%s\n", msg.data);

	*/
	//printf("gotoenclave->%I64d\n", eidcount[msg.id]);
	sgx_status_t i=hendlemessage_enclave_process(eidcount[msg.id], &msg);
	//sgx_status_t i = hendlemessage_enclave_process(eidcount[msg.id], &msg);
	printf("enclave->%I64dreturn %d\n", eidcount[msg.id],i);
	free(msg.data);
	...
	//printf("0x%d", msg.isActive);
	//printf("sucess to destroy enclave");
	return 0;
}
		...
		if (str == "quit") {
			while (1) {
				...
				for (int i = 0; i < httpServer::MAX; i++) {
					if (msg.isActive[i]) {
							if (SGX_SUCCESS != sgx_destroy_enclave(eidcount[i])) 
							{
									printf("fail to destroy enclave");
							}
							printf("sucess to destroy enclave");
...
bool httpServer::start()
{
	...
	for (int i = 0; i < httpServer::MAX; i++) {
		...
		sgx_status_t ret = SGX_SUCCESS;
		sgx_launch_token_t token = { 0 };
		ret = sgx_create_enclave(ENCLAVE_FILE, SGX_DEBUG_FLAG, &token, &updated, &eidcount[i], NULL);
		if (ret != SGX_SUCCESS)
		{
			printf("APP:ERROR%#x,fail to create a enclave space", ret);
			return -1;
